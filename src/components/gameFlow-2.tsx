...
function GameFlow() {
    ...
    function bet() {
        ...
        const playerBets : {idx: number; bet: number, hand: string[]}[] = [];
        ...
        let nxtPlayerIdx = currentPlayerIndex + 1;
        let tempBankAmount = bankAmount;
        while (nxtPlayerIdx < 3 && tempBankAmount > 0) {
            const currentPlayer = players[nxtPlayerIdx];

            const nextPlayer = players[currentPlayerIndex  + 1];
            const nextPlayerBet = window.prompt(`player ${nxtPlayerIdx + 1} enter your bet`);

            if(nextPlayerBet === null){
                // is bet pass, goes to the next player
                nxtPlayerIdx += 1;
                continue;
            }

            tempBankAmount =- Number(nextPlayerBet);


            playerBets.push({
                bet: Number(nextPlayerBet),
                idx: nxtPlayerIdx,
                hand: currentPlayer.hand,
            });
        ...
        // let tempBankAmount = 0;
        playerBets.forEach((currentPlayer) => {
        ...
        setBankAmount(tempBankAmount);


        // push the first player bet into the playerBet array;
